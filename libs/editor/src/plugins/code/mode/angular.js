(function(mod) {
    ...
    CodeMirror.defineMode("angular", function(config, modeConfig) {
        ...
        function nullToken(stream, state, cx) {
            ...
            if (stream.match('{{')) {
                ...
                state.context = new Context(CodeMirror.startState(jsMode), jsMode, 1, state.context)
        ...
        function xmlToken(stream, state, cx) {
            ...
            if (/\battribute\b/.test(style)) {
                ...
                if (/\*.*|\[.*]|\(.*\)|(bind|on|bindon)-.*/.test(cur)) {
                    ...
                    state.context = new Context(CodeMirror.startState(nullMode), nullMode, 0, state.context)
                }
            }
            if (/\bstring\b/.test(style)) {
                if (state.inBinding) {
                    stream.backUp(cur.length - 1)
                    state.context = new Context(CodeMirror.startState(jsMode), jsMode, 2, state.context)
                } else {
                    stream.backUp(cur.length - 1)
                    state.context = new Context(CodeMirror.startState(nullMode), nullMode, 1, state.context)
                }
                state.inAttr = false
        ...
        function jsToken(stream, state, cx) {
            ...
            if (stream.match(/template:\s?`/)) {
                state.context = new Context(CodeMirror.startState(xmlMode), xmlMode, 0, state.context)
            }

            if (stream.match(/styles:\s?\[\s?`/)) {
                state.context = new Context(CodeMirror.startState(cssMode), cssMode, 0, state.context)
            }
    ...
    CodeMirror.defineMIME("text/angular", {name: "angular", base: {name: "javascript", typescript: true}})
